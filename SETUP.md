...
function handleSaveProduct(e) {
  ...
  if (pass !== ADMIN_PASSWORD) return createJsonOutput({ success: false, message: 'Unauthorized' }, e.parameter.callback || '');
...
function handleAuth(e) {
  if (!e || !e.parameter) return createJsonOutput({ valid: false, message: 'No data' }, '');
  var valid = e.parameter.password === ADMIN_PASSWORD;
  var cb = e.parameter.callback || '';
  return createJsonOutput({ valid: valid, message: valid ? 'OK' : 'ভুল পাসওয়ার্ড' }, cb);
}
```
...
2. পাসওয়ার্ড: **mrinmoyee@2024** (পরিবর্তন করতে চাইলে Apps Script-এ `ADMIN_PASSWORD` ভেরিয়েবল এডিট করুন)
3. **প্রোডাক্ট লিস্ট** — সব প্রোডাক্ট দেখাবে
4. **+ নতুন প্রোডাক্ট** — নতুন প্রোডাক্ট যোগ করুন
5. **সম্পাদনা** — বিদ্যমান প্রোডাক্ট এডিট করুন
6. **ডিলিট** — প্রোডাক্ট মুছুন
